...
// src/handlers/mieaa_categories_handler.ts
var MiEAACategoriesHandler = class {
  async run(args) {
...
// src/handlers/mieaa_mirna_precursor_converter_handler.ts
function sleep(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms));
}
var MiEAAMirnaPrecursorConverterHandler = class {
  async run(args) {
    ...
    const input_type = direction === "mirna_to_precursor" ? "to_precursor" : "to_mirna";
    ...
    const text = await res.text();
    const input_label = direction === "mirna_to_precursor" ? "mirna" : "precursor";
    const output_label = direction === "mirna_to_precursor" ? "precursor" : "mirna";
...
// src/handlers/mieaa_mirbase_converter_handler.ts
function sleep2(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms));
}
var MiEAAMirBaseConverterHandler = class {
  async run(args) {
    ...
    const params = new URLSearchParams();
    mirnas.forEach((m2) => params.append("mirnas", m2));
    ...
    const text = await res.text();
    const apiResults = text.trim().split("\n").filter(Boolean).map((line) => {
      const [input, output] = line.split(/\t+/);
      return { input, output: output ?? null };
    });
    const conversions = mirnas.map((mirna) => {
      const hit = apiResults.find((r2) => r2.input === mirna);
      if (!hit) {
        return {
          input: mirna,
          output: mirna,
      ...
      if (hit.output === null) {
        return {
          ...
          output: null,
          status: "unmappable",
          reason: `No unambiguous miRBase mapping from ${from_version} to ${to_version}`
        };
      }
      return {
        input: mirna,
...
function formatMiEAAError(msg) {
  return { success: false, error: msg };
}
async function fetchMiEAACategories(species, inputType) {
...
async function startMiEAAJob(body, species, input_type, analysis_type) {
...
async function fetchMiEAAResults(jobId) {
      ...
      if (typeof raw === "object" && raw !== null) {
        if (raw.Enrichment_Results) {
          ...
          for (const cat of Object.keys(raw.Enrichment_Results)) {
            ...
            out2[cat] = rows.slice(0, LIMIT).map((r2) => ({
              category: cat,
              term: r2.term ?? r2.name ?? "",
              direction: r2.direction ?? "",
              es: r2.es,
              nes: r2.nes,
              p_value: r2.p_value,
              adj_p_value: r2.adj_p_value,
              leading_edge: r2.leading_edge,
              mirnas: r2.mirnas
            }));
          }
          return out2;
        }
        const looksLikeGSEA = Object.values(raw).some(
        ...
        for (const cat of Object.keys(raw)) {
          ...
          if (looksLikeGSEA && typeof rows[0] === "object") {
            out[cat] = rows.slice(0, LIMIT).map((r2) => ({
              category: cat,
              term: r2.term ?? r2.name ?? "",
              direction: r2.direction ?? "",
              es: r2.es,
...
// src/handlers/over_representation_analysis_handler.ts
var OverRepresentationAnalysisHandler = class {
  // -----------------------------------------------------
  // VALIDATION
  // -----------------------------------------------------
  validate(options) {
    ...
    for (let p of parts) {
      ...
      if (p.includes("-")) {
        ...
        for (let i2 = start; i2 <= end; i2++) {
          if (categories[i2 - 1]) chosen.add(categories[i2 - 1]);
        }
      } else {
  ...
  async run(options) {
    ...
    const startJob = await startMiEAAJob(
      body,
      options.species,
      options.entity,
      "ORA"
...
var oraTool = new OverRepresentationAnalysisHandler();
var categoryTool = new MiEAACategoriesHandler();
var mirnaPrecursorTool = new MiEAAMirnaPrecursorConverterHandler();
var mirbaseTool = new MiEAAMirBaseConverterHandler();
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  try {
    ...
    if (name === "over_representation_analysis") {
      return await oraTool.run(args);
    }
    if (name === "list_enrichment_categories") {
      return await categoryTool.run(args);
    }
    ...
    if (name === "mirbase_version_converter") {
      return await mirbaseTool.run({
        mirnas: args.mirnas,
        from_version: args.source_version,
